...
public class ViolationServiceImpl implements ViolationService {
    ...
    public void addUserViolation(AddingViolationsToUserDto add, MultipartFile[] multipartFiles, String email) {
        ...
        if (violationRepository.findActiveViolationByOrderId(order.getId()).isEmpty()) {
            ...
            eventService.saveEvent(OrderHistory.ADD_VIOLATION_UK, email, order.getId());
    ...
    public void deleteViolation(Long id, String uuid) {
        ...
        if (violationOptional.isPresent()) {
            ...
            eventService.save(OrderHistory.DELETE_VIOLATION_UK, currentUser.getEmail(),
                violationOptional.get().getOrder().getId());
        } else {
    ...
    public void updateUserViolation(UpdateViolationToUserDto add, MultipartFile[] multipartFiles, String uuid) {
        ...
        violationRepository.save(violation);
        eventService.saveEvent(OrderHistory.CHANGES_VIOLATION_UK, currentUser.getEmail(), violation.getOrder().getId());
        notificationService.notifyChangedViolation(violation.getId(), add.getOrderID());
    }
