...
class ChangedViolationNotificatorTest {
    ...
    void notifyByScheduleTest() {
        var schedule = "0 20 * * * *";
        var expectedDto = new ScheduledNotificationDto(CHANGED_IN_RULE_VIOLATION_STATUS, notificator.getClass());
        ...
        when(taskScheduler.scheduleNotification(runnableCaptor.capture(),
            eq(schedule), eq(CHANGED_IN_RULE_VIOLATION_STATUS))).thenReturn(mock(ScheduledFuture.class));
