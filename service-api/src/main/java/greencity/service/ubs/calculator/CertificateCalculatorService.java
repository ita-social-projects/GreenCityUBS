...
public interface CertificateCalculatorService {
    /**
     * Calculates the order amount in coins after applying all certificates
     * linked to the order (but before applying client-specific certificates).
     *
    ...
    /**
     * Applies client-specific certificates to the order and updates the amount to pay.
     * Certificates are applied after points,
     * since points must reduce the sum before certificates.
     * Certificates are marked as used if fully consumed. If a certificate's value
     * exceeds the remaining sum to pay, its balance is reduced accordingly and
     * the unused amount remains available for future orders.
     *
     * @param dto               request DTO containing payment/order details from client
